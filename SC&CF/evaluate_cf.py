...
def main():
	...
	rating_data = pd.read_table(srcPath, sep = ',', header = 0, index_col = 0)
	rating_data.columns = [int(x) for x in rating_data.columns.values]

	evaluate_result = evaluateCF(rating_data)
